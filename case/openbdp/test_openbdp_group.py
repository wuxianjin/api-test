...
import time
import random
from lib.iron_man.chart import Chart
from lib.iron_man.dashboard import Dshboard
from lib.iron_man.table import Table
...
class TestOpenbdpGroupCase:
    ...
    # 线上超管和auto_man token不一致暂时注释
    def test_openbdp_chartdata(self):
        openbdp = OpenbdpSdk.instance()
        t = Table.from_lines([
            "date,name,value,key",
        ...
        dsh_name = "openbdp" + str(time.time())

        d = Dshboard.create(dsh_name)

        c = Chart(dshobj=d, tb_obj=t)(
            x_axis=["name"],
            y_axis=["value"],
        )
        result = openbdp.call_method_and_succ("openbdp_chart_data", c.ct_id(), ret_expr='["result"]')
        assert result["data"] == [["A1", "9"], ["A2", "6"]]
        c.delete()
        d.delete()
