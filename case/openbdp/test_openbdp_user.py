...
class TestOpenbdpUserCase:
    ...
    def test_userfreeze(self):
        ...
        password = "123qweasd",
        role = 3
        uid = openbdp.call_method_and_succ("openbdp_userCreate", username, name, mobile, password, role, ret_expr='["result"]')
    ...
    def test_errchar(self):
        ...
        with pytest.raises(AssertionError):
            result = openbdp.call_method_and_succ("openbdp_userCreate",username, name, mobile, password, role, ret_expr='')
    ...
    def test_openbdp_userCreate(self):
        ...
        belong_groups = [gid,T]
        openbdp.openbdp_userCreate(username,name,mobile,password,role,email=email,position=position,belong_groups=belong_groups,has_dsh_permission=1,has_tb_permission=1,has_data_permission=1,has_dsh_manage_permission=1,has_date_filter_permission=1)
    ...
    def test_openbdp_manageCreate(self):
        ...
        account_manage = 1
        openbdp.openbdp_userCreate(username,name,mobile,password,role,email=email,position=position,belong_groups=belong_groups,manage_groups=manage_groups,account_manage=account_manage,tml_manage=tml_manage,function_manage=function_manage,tb_manage=tb_manage,data_source_manage=data_source_manage)
        uid = json.loads(openbdp.raw_data)["result"]
        result = openbdp.call_method_and_succ("openbdp_userInfo", uid, ret_expr='["result"]')
        assert result["username"] == username
        assert result["name"] == name
    ...
    def test_admin2user(self):
        ...
        account_manage = 1
        sex = 0
        openbdp.openbdp_userCreate(username,name,mobile,password,role,email=email,position=position,sex=sex,manage_groups=manage_groups,account_manage=account_manage)
        uid = json.loads(openbdp.raw_data)["result"]
        result = openbdp.call_method_and_succ("openbdp_userInfo", uid, ret_expr='["result"]')
        assert result["username"] == username
        assert result["name"] == name
    ...
    def test_user2admin(self):
        ...
        position = "modifposition"
        # userdmodify
        openbdp.openbdp_userdmodify(uid,name=name, mobile=mobile, email=email, position=position,role=role,sex=sex,manage_groups=manage_groups,account_manage=account_manage)
        result = openbdp.call_method_and_succ("openbdp_userInfo", uid, ret_expr='["result"]')
        assert result["name"] == name
        assert result["mobile"] == mobile
        assert result["role"] == role
    ...
    def test_ufunmodify(self):
        ...
        manage_groups = [pid]
        sex = 0
        openbdp.openbdp_userCreate(username,name,mobile,password,role,email=email,position=position,sex=sex,manage_groups=manage_groups,account_manage=1,tml_manage=1,function_manage=1,tb_manage=1,data_source_manage=1)
        uid = json.loads(openbdp.raw_data)["result"]
        result = openbdp.call_method_and_succ("openbdp_userInfo", uid, ret_expr='["result"]')
        assert result["username"] == username
        assert result["name"] == name
        ...
        assert result["tml_manage"] == 1
        # openbdp_ufunmodify
        openbdp.openbdp_ufunmodify(uid,account_manage=0,tml_manage=0,function_manage=0,tb_manage=0,data_source_manage=0)
        result = openbdp.call_method_and_succ("openbdp_userInfo", uid, ret_expr='["result"]')
        assert result["username"] == username
        assert result["account_manage"] == 0
        assert result["function_manage"] == 0
