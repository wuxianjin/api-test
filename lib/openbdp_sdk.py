...
class OpenbdpSdk:
    ...
    def __init__(self, access_token="", version=""):
        ...
        self.user_name = ""
        self.http_request = BDPRequest(bdpconf.OPENBDP_HOST, bdpconf.OPENBDP_PORT)
        if bdpconf.OPENBDP_PORT == 443:
    ...
    def instance(cls):
        if not cls._sdk_instance:
            cls._sdk_instance = OpenbdpSdk(bdpconf.OPENBDP_TOKEN, bdpconf.OPENBDP_VERSION)
    ...
    def openbdp_tb_down(self, task_id):
        req_param = {"task_id": task_id,
                     "access_token": bdpconf.OPENBDP_TOKEN
                     }
